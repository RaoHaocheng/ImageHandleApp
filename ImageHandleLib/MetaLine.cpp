...
int MetaLine::crossSearch(Point2i pts, float &angle, int ID)
{
	...
	if (edgeChain.size() >= 3)
	{
		float deltaX = (float)(edgeChain[0].x - edgeChain[edgeChain.size() - 1].x);
		float deltaY = (float)(edgeChain[0].y - edgeChain[edgeChain.size() - 1].y);
		angle = (float)atan(deltaY / deltaX + 0.00000101);
...
bool MetaLine::crossingCheck(Point2f pts, Point2f pte, int ID)
{
	...
	else
	{
		...
		float angle = (float)(CV_PI / 2.0 - atan(k));
...
void MetaLine::extendHirozontal(line_t &metaLineCur, lines_list_t  &metaLines, int *removal)
{
	...
	while (1)
	{
		...
		if (0 < xInt&&xInt < cols_1 && 0 < yInt&&yInt < rows_1)
		{
			...
			if (m0 < 0 && m0 != -ID) { size_t &n = lineHyps.push_back(); n = (size_t)(-m0 - 1); }
			if (m1 < 0 && m1 != -ID) { size_t &n = lineHyps.push_back(); n = (size_t)(-m1 - 1); }
			if (m2 < 0 && m2 != -ID) { size_t &n = lineHyps.push_back(); n = (size_t)(-m2 - 1); }
			if (lineHyps.size())
			{
				...
				if (Rnt != -1)
				{
					...
					if (leastSquareFitting(metaLineCur.points, newPara, sigma))
					{
						if (!newPara[0])
						{
							k = newPara[1];
							b = newPara[2];
							xInt = (int)metaLineCur.points[metaLineCur.points.size() - 1].x;
							yCur = k*xInt + b;
							removal[Rnt] = 1;
							extend = true;
						}
						...
						{
							metaLineCur.dir = (int)newPara[0];  //0 for k<1, 1 for k >=1
						...
						{
							int loc = (int)(metaLineCur.points[j].y*cols + metaLineCur.points[j].x);
							*(ptrMI + loc) = -float(ID);
						}
					}
					else
			...
			else
			{
				if ((m0 == 1 || m1 == 1 || m2 == 1) && (m0 + m1 + m2) == 1)
				{
					...
					else if (m1 == 1 && m1 >= m0&&m1 >= m2)
					{
						...
						p.x = (float)xInt;
						p.y = (float)yInt - 1;
						ptrMI[loc - cols] = (float)(-ID);
					}
					else if (m2 == 1 && m2 >= m0&&m2 >= m1)
					{
						if (chooseUp) { gap++; continue; };
						pixel_t &p = metaLineCur.points.push_back();
						p.x = (float)xInt;  p.y = (float)yInt + 1;
						ptrMI[loc + cols] = (float)(-ID);
					}
					edge++;
					edgeTotal++;
	...
	if (extend)
	{
		...
		metaLineCur.dir = (int)metaLineParas[0];  //0 for k<1, 1 for k >=1
				...
				if (Rnt != -1)
				{
					...
					if (leastSquareFitting(metaLineCur.points, newPara, sigma))
					{
						if (newPara[0])
						{
							k = newPara[1];
							b = newPara[2];
							yInt = (int)metaLineCur.points[metaLineCur.points.size() - 1].y;
							xCur = k*yInt + b;
							removal[Rnt] = 1;
							extend = true;
						}
...
void MetaLine::getInformations(cv::Mat &originalImage, float gausSigma, int gausHalfSize, float p)
{
	...
	int grayLevels = 255;
	int apertureSize = 3;
	float anglePer = (float)(CV_PI / 8.0);
	...
	float pMax = (float)(1.0 / exp((log(N2) / thMeaningfulLength)));
	float pMin = (float)(1.0 / exp((log(N2) / sqrt((float)cols*rows))));
...
bool MetaLine::smartRouting(clusters_list_t &segments, float minDeviation, int minSize)
{
	...
	for (i = 0; i < numGradientPoints; ++i)
	{
		...
		pixel_t temp;
		for (m = 0, n = (int)str.size() - 1; m < n; ++m, --n)
		{
			temp = str[m];
			str[m] = str[n];
			str[n] = temp;
	...
	{
		int R = (int)colors[(i % 7)].val[0];
		int G = (int)colors[(i % 7)].val[1];
		int B = (int)colors[(i % 7)].val[2];
		for (j = 0; j < strings[i].size(); ++j)
		{
			int x = (int)strings[i][j].x;
			int y = (int)strings[i][j].y;
		...
		char text[100];
		sprintf(text, "%d", i);
		Point mid = Point((int)strings[i][strings[i].size() / 2].x, (int)strings[i][strings[i].size() / 2].y);
	...
	for (i = 0; i < segments.size(); ++i)
	{
		...
		{
			int x = (int)(times*segments[i].pixels[j].x);
			int y = (int)(times*segments[i].pixels[j].y);
...
void MetaLine::getMetaLine(clusters_list_t &segments, lines_list_t &metaLines, float sigma)
{
	...
	for (i = 0; i < numSegments; i++)
	{
		...
		for (j = 0; j < segments[i].size; ++j)
		{
			...
			int loc = (int)(segments[i].pixels[j].y*cols + segments[i].pixels[j].x);
			ptrMaskImage[loc] = numLines;
		}

		metaLineTemp.ID = (int)-numLines;
		metaLineTemp.dir = (int)parameters[0];  //0 for k<1, 1 for k >=1
...
void MetaLine::MetaLineDetection(cv::Mat originalImage, float gausSigma, int gausHalfSize, std::vector<std::vector<float> > &lines)
{
	...
	for (i = 0; i < metaLines.size(); ++i)
	{
		if (!removal[i])
		{
			...
			lineTemp[4] = (float)metaLines[i].ID;
...
{
	int i;
	int numCur = (int)metaLineCur.points.size();

	float angleCur = 0.0;
	if (metaLineCur.points[0].x == metaLineCur.points[numCur - 1].x)
	{
		angleCur = (float)(CV_PI / 2.0);
	}
	else
	...
	for (i = 0; i < lineHyps.size(); ++i)
	{
		float angleOffset =(float) min((double)abs(angles[i] - angleCur), CV_PI - abs(angles[i] - angleCur));
		if (angleOffset < angleMin)
		{
			angleMin = angleOffset;
			metaIDHyp = (int)lineHyps[i];
		}
	}
	if (angleMin > thAngle)
		return -1;
	else
	{
		//merged the line
		float thDis = 4;
		int numHyp = (int)metaLines[metaIDHyp].points.size();
...
int MetaLine::lineMerging2(int IDCur, line_t &metaLineCur, size_list_t &lineHyps, lines_list_t &metaLines, size_list_t &IDMerged)
{
	...
	float length = (float)metaLineCur.points.size();
	...
	for (i = 0; i < lineHyps.size(); ++i)
	{
		if (!removal[i])
		{
			...
			IDHypsNew.push_back((int)lineHyps[i]);
	...
	if (IDHypsNew.size())
	{
		...
		QuickSort<float, int>::SortAscent(&dis[0], 0, (long)dis.size() - 1, &IDHypsNew[0]);
		...
		for (i = 0; i < IDHypsNew.size(); ++i)
		{
			...
			if (NFA > 1)
			{
				...
				if (disMin == dis1)
				{
					...
					for (j = (int)length - 1; j >= 0; --j) { mergedPoints.push_back(Point2f(metaLineCur.points[j].x, metaLineCur.points[j].y)); }
				...
				else if (disMin == dis2)
				{
					...
					for (j = (int)length - 1; j >= 0; --j)     { mergedPoints.push_back(Point2f(metaLineCur.points[j].x, metaLineCur.points[j].y)); }
					for (j = (int)lengthCur - 1; j >= 0; --j)  { mergedPoints.push_back(Point2f(metaLines[IDHypsNew[i]].points[j].x, metaLines[IDHypsNew[i]].points[j].y)); }
				}
				else if (disMin == dis3)
				{
					pts = Point2f(xe, ye);
				...
				else
				{
					pts = Point2f(xe, ye);
					pte = Point2f(xeCur, yeCur);
					for (j = 0; j < (int)length; ++j)       { mergedPoints.push_back(Point2f(metaLineCur.points[j].x, metaLineCur.points[j].y)); }
					for (j = (int)lengthCur - 1; j >= 0; --j) { mergedPoints.push_back(Point2f(metaLines[IDHypsNew[i]].points[j].x, metaLines[IDHypsNew[i]].points[j].y)); }
				}
...
{
	int i, m, n;
	int num = (int)metaLines.size();

	//sort lines
	std::vector<int> length;
	std::vector<int> index;
	for (i = 0; i < (int)metaLines.size(); ++i)
	{
		if (metaLines[i].points.size()>2 * thMeaningfulLength)
		{
			index.push_back(i);
			length.push_back((int)metaLines[i].points.size());
		}
	}
	QuickSort<int, int>::SortDescent(&length[0], 0, (long)length.size() - 1, &index[0]);
	...
	for (i = 0; i < length.size(); ++i)
	{
		...
		if (!removal[t])
		{
			...
			if (!dir)  //horizontal line
			{
				...
				for (m = 0, n = (int)metaLines[t].points.size() - 1; m < n; ++m, --n)
				{
			...
			metaLines[t].dir = (int)metaLineParas[0];  //0 for k<1, 1 for k >=1
...
void MetaLine::metaLineMerging(lines_list_t &metaLines, int *removal)
{
	...
	//creat line bins
	int numBins = 16;
	float stepBins = (float)(CV_PI / numBins);
	...
	for (i = 0; i < length.size(); ++i)
	{
		...
		angle +=(float) CV_PI / 2.0;
		int binID =(int) (angle / stepBins);
...
void MetaLine::subDivision(clusters_list_t &clusters, const string_t &string, const size_t first_index, const size_t last_index, const float min_deviation, const size_t min_size)
{
	...
	int x =(int)( first.x - last.x);
	int y = (int)(first.y - last.y);
	...
	int half_min_size = (int)min_size / 2;
...
float MetaLine::lineValidityCheckGradientLevel(line_t &metaLines)
{
	...
	int numPoints = (int)metaLines.points.size();
	...
	{
		int loc = (int)(metaLines.points[j].y*cols + metaLines.points[j].x);
		gradient.push_back(ptrGM[loc]);
	}

	//sort
	QuickSort<float, int>::SortDescent(&gradient[0], 0, (int)gradient.size() - 1);
...
float MetaLine::lineValidityCheckGradientOrientation(line_t &metaLines)
{
	...
	float angleOffset = (float)(CV_PI / 8.0f);
